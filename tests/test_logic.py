...
def test_health_reports_rev_and_ws(tmp_path):
    ...
    assert body.get("twap_horizons") == ["5m", "15m"]
...
def test_strategy_mode_of_defaults_to_twap():
    ...
    assert DEFAULT_SETTINGS["strategy_rev"] == 30
...
def test_rev27_opens_280s_and_eth_keeps_paper(tmp_path):
    ...
    assert s["twap_horizons"] == ["5m", "15m"]
    assert s["live_trading"] is False
    assert "sol" in DEFAULT_SETTINGS["twap_assets"]
    assert DEFAULT_SETTINGS["twap_horizons"] == ["5m", "15m"]
    p = default_params(s)
    assert p.max_left == 280.0
...
def test_ws_wanted_tokens_skips_future_and_far_15m():
    ...
    got = ws_wanted_tokens(events, params=params, hold_condition_ids={"chold"}, extra_tokens=["restu"])
    ...
    assert "u15h" in got and "d15h" in got
    assert "restu" in got
    assert len(got) <= 8
