...
def test_live_leftover_paper_does_not_block_twap(tmp_path):
    ...
    assert "實盤今日" in home_text(rt)
    assert "紙盤帳" in home_text(rt)
    log = _log_text(rt)
...
def test_strategy_mode_of_defaults_to_twap():
    ...
    assert DEFAULT_SETTINGS["strategy_rev"] == 41
